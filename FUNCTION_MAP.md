...
desglosarNombre(texto)
    ...
    Es usado por: app.js (varios), sugerencias-print.js (la usa como fallback si no existe).
...
window.desglosarNombre(texto) (Utilidad compartida inyectada)
    ...
    Contrato defensivo: Solo se inyecta en window si previamente no existe (para evitar sobrescribir la de app.js).
    Retorna: { nombre: String, uvas: String }
    Es usado por: Internamente en este archivo.
